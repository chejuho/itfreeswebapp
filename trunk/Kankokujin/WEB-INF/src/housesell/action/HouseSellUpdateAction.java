...
import housesell.handler.HouseSellUpdateHandler;
...
public class HouseSellUpdateAction extends HttpServlet {
	/**
	 * 
	 */
	...
	public void service(HttpServletRequest request, HttpServletResponse response)
		...
		KankokujinLogger.getInstance().debug("HouseSellUpdateAction.start");
		HouseSellUpdateHandler houseSellUpdateHandler = HouseSellUpdateHandler.getInstance();
		HouseBean houseBean = new HouseBean();
		HouseSearchBean houseSearchBean = new HouseSearchBean();
		MemberBean member = (MemberBean) request.getSession().getAttribute("memberInfo");
		houseBean.setUser_id(member.getUserid());		
		boolean result = false;
		try {
			setHouseBean(houseBean, houseSearchBean, request);
			result = houseSellUpdateHandler.updateHouseSellBean(houseBean);
		} catch (AppException e) {
			throw new KankokujinException("HouseSellUpdateAction", e);
		} catch (Exception e) {
			throw new KankokujinException("HouseSellUpdateAction", e);
		} finally {
		}
		...
		if (result) {
			...
			this.getServletContext().getRequestDispatcher(
					"/jsp/housesell/houseSellDetail.jsp").forward(request,
					response);
		} else {
			this.getServletContext().getRequestDispatcher("/HouseSellList")
