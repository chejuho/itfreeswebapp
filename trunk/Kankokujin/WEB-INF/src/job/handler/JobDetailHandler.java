...
public class JobDetailHandler {
	private static JobDetailHandler instance = null;
	private JobDetailHandler(){
		
	}
    public static JobDetailHandler getInstance() {
        if (instance == null) {
            synchronized (JobDetailHandler.class) {
                if (instance == null) {
                    instance = new JobDetailHandler();
                }
            }
        }
        return instance;
	 ...
	 * @throws AppException
	 */
	public JobBean getJobBean(JobBean bean, boolean updateflg, boolean brflg) throws SysException {
		...
		try {
			...
			if (updateflg){
				String strSql = "update job_info set read_count = read_count + 1 where id = ?";
				pstmt1 = new LogPreparedStatement(con, strSql);
				pstmt1.setString(1, bean.getId());
				int result = pstmt1.executeUpdate();
				KankokujinLogger.getInstance().debug("JobDetailHandler.getJobBean.sql1=" + pstmt1.getQueryString());
				if (result == 1) {
					//return bean;
				}
			}
			StringBuffer sb = new StringBuffer("select * from job_info, member, t_job_category, t_area , t_train where job_info.id = ? and ");
			sb.append("job_info.user_id=member.user_id and  job_info.area_code_1=t_area.area_code_1 and  ");
			sb.append("job_info.area_code_2=t_area.area_code_2 and  job_info.line_code=t_train.line_code and  ");
			sb.append("job_info.station_code=t_train.station_code and ");
			sb.append("job_info.cate_code_1=t_job_category.cate_code_1 and job_info.cate_code_2=t_job_category.cate_code_2");
			pstmt2 = new LogPreparedStatement(con, sb.toString());
			pstmt2.setString(1, bean.getId());
			rs = pstmt2.executeQuery();
			KankokujinLogger.getInstance().debug("JobDetailHandler.getJobBean.sql2=" + pstmt2.getQueryString());
			if (rs.next()) {
				...
				bean.setSort_code(Util.changeNullStr(rs.getString("sort_code")));
				...
				bean.setCharge(Util.changeNullStr(rs.getString("charge")));
