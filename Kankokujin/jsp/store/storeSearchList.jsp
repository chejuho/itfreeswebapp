...
<head>
<title><c:out value="업체찾기"/>&nbsp;::&nbsp;<fmt:message key="TITLE.title" bundle="${message}"/></title>
<meta http-equiv="Content-type" content="text/html; charset=UTF-8">
...
<body style="margin:0 0 0 0">
	...
	<tr>
		<td>
			<form name="list" method="post" style="margin:0" action="StoreSearch">
			<input type="hidden" name="user_id" value="${StoreSearchBean.user_id}">      			
			<input type="hidden" name="before_cate_code_1" value="${StoreSearchBean.cate_code_1}">
			<input type="hidden" name="before_cate_code_2" value="${StoreSearchBean.cate_code_2}">       			    
	<!--		<input type="hidden" name="before_pageNum" value="${StoreSearchBean.pageNum}"> 
			<input type="hidden" name="before_pageSize" value="${StoreSearchBean.pageSize}">-->
			...
			<tr>
				<td>
					...
					<tr>
						<td height="71" colspan="3" background="jsp/images/new/com_search_tit.gif">
							...
							<tr> 
            					...
            					<td width="83%" rowspan="2">
            						<c:forEach var="Category1" items="${Category1List}" varStatus="i">
          								<c:if test="${StoreSearchBean.cate_code_1 eq Category1.cate_code}">
										...
										<c:if test="${StoreSearchBean.cate_code_1 != Category1.cate_code}">
							...
							<tr>
        						<!--제1카테고리검색수-->
          						<td height="25" class="search-box-num style5" >
									<c:out value="(${Category1Count['categoryTotalCount']})"/>
          						</td>
        					</tr>
							</table>
						</td>		
					...
					<tr>
						...
						<td width="910" align="center" valign="middle" style="padding:10px 8px 8px 8px;">
							...
							<c:if test="${!empty StoreSearchBean.cate_code_1}">
				            <tr>
				            	<!--제2카테고리-->
				                <td width="19%" align="left"><!--선택된제1카테고리-->
					            ...
					            <td width="70%" align="left">
									<c:forEach var="Category2" items="${Category2List}" varStatus="i">
          								<c:if test="${StoreSearchBean.cate_code_2 eq Category2.cate_code_2}">
											<span class="search-text3" onclick="return reloadCate_code_2('${StoreSearchBean.cate_code_1}','${Category2.cate_code_2}')"> 
												<!--카테고리명  검색건수 표시-->
										...
										<c:if test="${StoreSearchBean.cate_code_2 != Category2.cate_code_2}">
											<a href="javascript:reloadCate_code_2('${StoreSearchBean.cate_code_1}','${Category2.cate_code_2}')" class="search-link2">
												<!--카테고리명  검색건수 표시-->
												<c:if test="${empty Category2Count[Category2['cate_code_2']]}">
													<c:out value="${Category2.cate_name_2}(0)"/>
												</c:if>
							...
							<tr>
								<td colspan="2" align="left">
									<!--지역 단위 1-->
									<select name="search_area_code_1" onChange="return reloadAreaLineSearch('0');">
			                			${StoreSearchBean.area_1Tag}
									</select>
									<!--지역 단위 2-->
									<select name="search_area_code_2">
										${StoreSearchBean.area_2Tag}
									</select>
							  		&nbsp;&nbsp;&nbsp;&nbsp;
									<!--라인-->
									<select name="search_line_code" onChange="return reloadAreaLineSearch('1');">
										${StoreSearchBean.lineTag}
									</select>
					                 <!--역-->
									<select name="search_station_code">
										${StoreSearchBean.stationTag}
									</select>
							  		&nbsp;&nbsp;&nbsp;&nbsp;
									<!--전체검색-->
									<select name="search_range" id="select7">
										<option value='0' ${StoreSearchBean.search_all}>전체검색</option>
										<option value='1' ${StoreSearchBean.search_com_name}>업체명</option>
										<option value='2' ${StoreSearchBean.search_detail}>상세정보</option>
									</select>
									<input type="text" name="search" id="textfield2" value='${StoreSearchBean.search}'/></td>
								<td width="11%" align="right">
									<a href="StoreSearch?re=9"><img src="jsp/images/new/search_return_b.gif" width="94" height="22" /></a></td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td colspan="3" align="center">
									<a href="javascript:searchStore('${StoreSearchBean.cate_code_1}','${StoreSearchBean.cate_code_2}');"><img src="jsp/images/new/search_b.gif" width="107" height="35" /></a></td>
							</tr>
							</table>
						</td>
			...
			<tr>
				<td>
					...
					<tr>
						<td width="13%">
							<c:if test="${empty StoreSearchBean.user_id}">
					...
					<tr>
						<td height="41" colspan="7" background="jsp/images/new/board_list_top_bg.gif" class="search-list-tit">
							...
							<tr>
								...
								<td width="39%" align="center" class="board-title-text"><span class="style8"> 업체명</span></td>
						...
						<c:forEach var="StoreBean" items="${StoreBeanList}" >
							<tr>
					             ...
					             <td width="8%" align="center">
					             	<a href="javascript:openDetailPage('${StoreBean.id}');"><img src="${StoreBean.thumbNail_path}" ></a>
					             </td>
					              <!--업체명 -->
					             <td width="39%" valign="center">
					             	<a href="javascript:openDetailPage('${StoreBean.id}');"><c:out value="${StoreBean.store_full_name}"/>${util:getNewImage(StoreBean.update_dateTime)}</a>
									&nbsp;
					             </td>
					              <!--전화번호 -->
