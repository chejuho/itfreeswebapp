...
<%@ page import="store.bean.StoreBean"%>
				...
				<!-- 여기에 실제 메인 바디 소스코딩을 해주세요. -->	
					...
					<tr><td>
							...
							<table border="0" cellpadding="0" cellspacing="0"
								...
								<%
									Iterator storeBeanListIt = storeBeanList.iterator();
									int storeBeanListSize = storeBeanList.size();
									if (storeBeanListIt.hasNext()) {
										while (storeBeanListIt.hasNext()) {
											StoreBean bean = new StoreBean();
											bean = (StoreBean) storeBeanListIt.next();
								%>
									<tr height="28">
										<td><a href="StoreDetail?id=<%=bean.getId()%>&cate_code_1=<%=bean.getCate_code_1()%>&cate_code_2=<%=bean.getCate_code_2()%>"><%=bean.getId()%></a></td>
										<td><a href="StoreDetail?id=<%=bean.getId()%>&cate_code_1=<%=bean.getCate_code_1()%>&cate_code_2=<%=bean.getCate_code_2()%>"><img src="<%=bean.getPhoto_path1()%>" width="50" height="50"></a></td>
										<td><a href="StoreDetail?id=<%=bean.getId()%>&cate_code_1=<%=bean.getCate_code_1()%>&cate_code_2=<%=bean.getCate_code_2()%>"><%=bean.getStore_name_k()%></a></td>
