...
import engmgn.handler.EngMgnRegistHandler;

public class EngMgnRegistAction extends HttpServlet {
	...
	public void service(HttpServletRequest request, HttpServletResponse response)
		...
		try{
			KankokujinLogger.getInstance().debug("EngMgnRegistAction.start");
			System.out.println("EngMgnRegistAction");
			EngMgnRegistHandler handler = new EngMgnRegistHandler();
			
			EngMgnBean bean = setEngMgnBean(request);

			result = handler.insert(bean);

		
		} catch (AppException e) {
			throw new KankokujinException("StoreRegistAction", e);
		} catch (Exception e) {
			throw new KankokujinException("EngMgnRegistAction", e);
		} finally {
			KankokujinLogger.getInstance().debug("EngMgnRegistAction.end");
		}
		if (result) {
			...
			this.getServletContext().getRequestDispatcher(
			"/EngMgnRegistOpen").forward(request, response);
		}
	}
