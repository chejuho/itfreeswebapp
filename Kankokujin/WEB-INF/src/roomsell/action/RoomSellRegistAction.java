...
import roomsell.handler.RoomSellRegistHandler;
...
public class RoomSellRegistAction extends HttpServlet {
	...
	public void service(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			KankokujinLogger.getInstance().debug("RoomSellRegistAction.start");
			RoomSellRegistHandler roomSellRegistHandler = RoomSellRegistHandler.getInstance();
			setRoomBean(roomBean, roomSearchBean, request);
			result = roomSellRegistHandler.insertRoomBean(roomBean);
			roomSearchBean.setPageNum("");
			imageProcess(roomBean);
		} catch (AppException e) {
			throw new KankokujinException("RoomSellRegistAction", e);
		} catch (Exception e) {
			throw new KankokujinException("RoomSellRegistAction", e);
		} finally {
			KankokujinLogger.getInstance().debug("RoomSellRegistAction.end");
		}
		if (result) {
			request.setAttribute("RoomSearchBean", roomSearchBean);
			this.getServletContext().getRequestDispatcher("/jsp/roomsell/roomSellRegistResult.jsp").forward(request, response);

		} else {
			this.getServletContext().getRequestDispatcher(
					"/RoomSellRegistOpen").forward(request, response);
		}		
	}
